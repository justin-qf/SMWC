...
import com.vovance.omcsalesapp.Common.PubFun.Companion.removeSpaceFromText
...
import com.vovance.securemywillcall.databinding.ActivityCompanyInfoBinding
    ...
    private fun apiCall() {
        if (Utils.hasNetwork(act)) {
            ...
            addCompanyViewModel.addCompany(
                addCompanyParam,
                if (prefManager.getUser()!!.token!!.isNotEmpty()) getString(R.string.serverErrorMessage)  + prefManager.getUser()!!.token!! else ""
            )
        } else {
